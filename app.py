...
from sales_item import SaleItem
...
if menu_option == "Dashboard":
    ...
    try:
        # Count customers
        cur = conn.cursor()
        cur.execute('SELECT COUNT(*) FROM coustmer')
...
# Customer Management
    ...
    if customer_action == "View All Customers":
        ...
        try:
            customers = coustmer.get_all_coustmer()
            if customers:
            ...
            if submitted:
                if name and contact:
                    try:
                        ...
                        # Refresh sequence
                        cur = conn.cursor()
                        cur.execute("SELECT setval('coustmer_id_seq', (SELECT MAX(id) FROM coustmer))")
                        conn.commit()
                        cur.close()
                    except Exception as e:
        ...
        try:
            customers = coustmer.get_all_coustmer()
            if customers:
                coustmer_ids = [customer[0] for customer in customers]
                selected_id = st.selectbox("Select Customer to Update", coustmer_ids)
                
                if selected_id:
                    # Get current customer data
                    cur = conn.cursor()
                    cur.execute('SELECT * FROM coustmer WHERE id = %s', (selected_id,))
                    ...
                    if customer:
                        with st.form("update_customer_form"):
                            ...
                            if submitted:
                                try:
                                    coustmer.update_coustmer(selected_id, name, contact)
            ...
            if customers:
                ...
                if st.button("Delete Customer"):
                    if selected_customer:
                        coustmer_id = customer_options[selected_customer]
                        try:
        ...
        try:
            ...
            if products:
                ...
                if selected_id:
                    ...
                    product = Products.get_product_by_id(selected_id)
                    ...
                    if product:
                        with st.form("update_product_form"):
                            ...
                            if submitted:
                                try:
                                    Products.update_product(selected_id, name, description, price, quantity)
    ...
    if sales_action == "Create New Sale":
        ...
        try:
            ...
            if customers:
                ...
                if selected_customer:
                    coustmer_id = customer_dict[selected_customer]
                    ...
                    with st.form("create_sale_form"):
                        ...
                        if submitted: 
                            try:
                                # Insert sale
                                cur = conn.cursor()
                                cur.execute('''INSERT INTO sales (coustmer_id, date, total_amount)
                                               VALUES (%s, %s, %s) RETURNING id''',
                                            (coustmer_id, sale_date, 0.0))
                                    ...
                                    with st.form("add_item_form"):
                                        ...
                                        if submitted_item:
                                            try:
                                                ...
                                                Products.update_product(product_id, quantity=new_quantity)
        ...
        try:
            ...
            cur.execute('''SELECT s.id, c.name, s.date, s.total_amount 
                          FROM sales s JOIN coustmer c ON s.coustmer_id = c.id
            ...
            if customers:
                ...
                if selected_customer:
                    ...
                    cur.execute('''SELECT s.id, s.date, s.total_amount 
                                  FROM sales s 
                                  WHERE s.coustmer_id = %s 
                                  ORDER BY s.date DESC''', (coustmer_id,))
